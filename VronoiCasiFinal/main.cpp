...
vector<Polygon> delaunayTriangulation(vector<Point>& points) {
    ...
    while (!points.empty()) {
        ...
        cout << "Punto acutal a agregar" << endl;
        cout << puntoAgregar.getX() << " " << puntoAgregar.getY() << endl << endl;
        ...
        cout << "Numero de triangulos invalidos encontrados: " << count2 <<endl<<endl;
        ...
        //aristasComunes = findSharedEdges(triangulosInvalidos[0], triangulosInvalidos);

        cout << "Aristas comunes" << endl;
        for (auto i: aristasComunes) {
            i.print();
        }
        cout << endl;
        ...
        cout << "Aristas invalidas" << endl;
        for (auto i: aristasInvalidas) {
            i.print();
        }
        cout << endl;
        ...
        for (int i=0; i<triangulosInvalidos.size(); i++) {
            ...
            // aristasComunes += findSharedEdges(triangulosInvalidos[i], triangulosInvalidos);
                    // vector<Segment> segments = findSharedEdges(triangulosInvalidos[i], triangulosInvalidos);
                    // aristasComunes.insert(aristasComunes.end(), segments.begin(), segments.end());
        ...
        cout << "PUNTOS INVALIDOS: "<< endl;
        for (auto i: puntosInvalidos) {
            cout << i.getX() << " " << i.getY() << endl;
        }
        cout << endl;

        cout << "Iteracion: " << count <<" ---------- ANTES"  << endl;
        for (int i=0; i<triangulosDT.size(); i++) {
            vector<Point> pointsTriangle = triangulosDT[i].getPoints();
            cout << "Triangulo " << i << endl;
            for (int j=0; j<3; j++) {
                cout << "("<< pointsTriangle[j].getX() << ", " << pointsTriangle[j].getY()<< ")" << endl;
            }
            cout << endl;
        }
        ...
        for (pi = puntosInvalidos.begin(); pi != puntosInvalidos.end(); pi++) {
            ...
            for (pj = next(pi); pj != puntosInvalidos.end(); pj++) {
                ...
                //!isEdgeInTriangles(Segment(*pi,*pj), aristasComunes) && isEdgeInTriangles(Segment(*pi,*pj), aristasInvalidas)
                if (!isEdgeInTriangles(Segment(*pi,*pj), aristasComunes) && isEdgeInTriangles(Segment(*pi,*pj), aristasInvalidas)) { //&& it == aristasComunes.end()
                    // Hacer triángulos_DT.append((pi,pj,punto_a_añadir))
                    cout << "Segment passed: " << endl;
                    Segment s = Segment(*pi, *pj);
                    s.print();
                    cout << endl;
        ...
        //DEBUGGING
        count++;
        cout << "Iteracion: " << count <<" ---------- FINAL"  << endl;
        for (int i=0; i<triangulosDT.size(); i++) {
            vector<Point> pointsTriangle = triangulosDT[i].getPoints();
            cout << "Triangulo " << i << endl;
            for (int j=0; j<3; j++) {
