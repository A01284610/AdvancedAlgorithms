...
void GrafoKruskal::Kruskal() {
    ...
    for (int i = 0; i < T.size(); i++) {
        sum += T[i].first;
        cout << "Del nodo: " << T[i].second.first << " al nodo: " << T[i].second.second << " Costo: " << T[i].first << endl;
    }
...
void getFileText(){
    ...
    // Se inicializa un objeto tipo 'GrafoKruskal' con N en el constructor representando la cantidad de nodos.
    //GrafoKruskal g(N);

    string S;
    getline(cin, S);

    // Se inicializa variable string 'myText' que sera utilizada para obtener las N lineas y sus valores según la diagonal menor (considerando un grafo no dirigido).
    string myText;
    ...
    for (int i = 0; i < N; i++){
        ...
        for (int j=0; j < N; j++){
            ...
            datos.pesos.AgregarArista(i, j, stoi(val));
    ...
    for (int i = 0; i < N; i++){
        ...
        for (int j=0; j < N; j++){
            ...
            string val = text.substr(0,pos);
            text = text.substr(pos+1);
            datos.flujos.addEdge(i, j, stoi(val));

        }
    }
    cout << datos.flujos.DinicMaxflow(0,2) << endl;
    ...
    for (int i = 0; i < N; i++){
        ...
        // Variable para saber la posicion de cada ',' entre los N valores.
        int pos = 0;
        // Se separan y obtienen los valores del par.
        pos = text.find(" ");
        int val1 = stoi(text.substr(0,pos));
        int val2 = stoi(text.substr(pos+1));
    }
