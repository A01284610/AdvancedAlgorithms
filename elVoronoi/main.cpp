...
vector<Polygon> delaunayTriangulation(vector<Point>& points) {
    ...
    while (!points.empty()) {
        ...
        for (std::vector<Polygon>::size_type i=0; i<triangulosInvalidos.size(); i++) {
        ...
        cout << "BBBB " << triangulosDT.size() << " BBB" << endl;
        cout << "CCCC " << triangulosInvalidos.size() << " CCC" << endl;
        cout << "AAAA " << puntosInvalidos.size() << " BBB" << endl;
